    ...
    if offset > 0 {
        ...
        if permissions.mode() & 0o777 != 0o600 {
            return Err(std::io::Error::new(
                ...
                "Insecure file permissions detected on existing file. Cannot append.",
            ));
        }
    }
...
mod tests {
    ...
    async fn test_open_secure_file_append_insecure_fails() {
        ...
        // 2. Append using open_secure_file (should fail because file is 0o666)
        let result = open_secure_file(&file_path, 10).await;

        #[cfg(unix)]
        {
            assert!(result.is_err(), "open_secure_file should return an error when appending to insecurely permissioned file");
            let err = result.unwrap_err();
            assert_eq!(err.kind(), std::io::ErrorKind::PermissionDenied);
        }
        #[cfg(not(unix))]
        {
            // On non-unix it just opens the file normally since mode() is not available
            assert!(result.is_ok());
        }
