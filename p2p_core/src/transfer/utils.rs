    ...
    if offset > 0 {
        ...
        if metadata.permissions().mode() & 0o777 != 0o600 {
            return Err(std::io::Error::new(
                ...
                "Insecure file permissions: existing file must be 0o600",
            ));
        }
    }
...
mod tests {
    ...
    async fn test_open_secure_file_append_insecure_fails() {
        ...
        // 2. Append using open_secure_file (should fail due to permissions)
        let result = open_secure_file(&file_path, 10).await;

        #[cfg(unix)]
        assert!(result.is_err(), "Appending to insecure file should fail");
