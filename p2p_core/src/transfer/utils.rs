    ...
    if offset > 0 {
        ...
        if metadata.permissions().mode() & 0o777 != 0o600 {
            return Err(std::io::Error::new(
                ...
                "File permissions tampered with",
            ));
        }
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_open_secure_file_append_permissions_tampered() {
        let temp_dir = std::env::temp_dir();
        let file_path = temp_dir.join(format!(
            "secure_append_tampered_test_{}.txt",
            uuid::Uuid::new_v4()
        ));
        ...
        // 2. Try to append using open_secure_file (offset > 0)
        let result = open_secure_file(&file_path, 10).await;

        // 3. Verify it returns an error on Unix due to tampered permissions
        #[cfg(unix)]
        {
            assert!(result.is_err());
            let err = result.unwrap_err();
            assert_eq!(err.kind(), std::io::ErrorKind::PermissionDenied);
            assert_eq!(err.to_string(), "File permissions tampered with");
        }
