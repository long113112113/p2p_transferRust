...
## 2024-05-25 - TOCTOU Race Condition in File Creation
**Vulnerability:** In `p2p_core/src/identity.rs`, the `node_secret.key` file was created using an insecure `if path.exists()` check followed by file creation with `options.write(true).create(true).truncate(true)`. An attacker could pre-create this file between the check and creation with permissive permissions (e.g., world-readable), defeating the intended 0o600 restrictions since `OpenOptions::mode()` only applies to newly created files.
**Learning:** Checking for file existence before creation introduces a Time-of-Check to Time-of-Use (TOCTOU) vulnerability. This is a common pattern that is insecure when dealing with sensitive files like cryptographic keys.
**Prevention:** Always use atomic file creation by specifying `create_new(true)` on `OpenOptions`. This ensures that the operation will fail if the file already exists, eliminating the window for a TOCTOU race condition and guaranteeing that the correct permissions are applied upon creation.
