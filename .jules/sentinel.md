...
## 2026-08-05 - File Permission Persistence on Append/Resume
**Vulnerability:** The `open_secure_file` function in the P2P transfer module relied on `OpenOptions::mode(0o600)` to set permissions on new files, but when `offset > 0` (resuming a transfer), it merely appended to the existing file without enforcing its permissions. If an attacker pre-created the file with permissive modes (e.g., world-readable `0o666`), the resumed file retained those insecure permissions, exposing the downloaded data.
**Learning:** `OpenOptions::mode` in Rust only sets permissions when *creating* a new file. When opening an existing file for appending or resuming, its previous permissions are retained. Furthermore, attempting to "fix" permissions post-open via `set_permissions` is flawed; it fails if the attacker owns the pre-created file (yielding `EPERM`) and doesn't revoke handles already opened by an attacker, leading to a false sense of security.
**Prevention:** When resuming or appending to existing files, securely check the file's metadata (permissions) *before* proceeding. If the file's permissions have been tampered with or do not match the strict expected mode (e.g., `0o600`), immediately return an error and refuse the transfer rather than attempting an insecure post-open fix.
