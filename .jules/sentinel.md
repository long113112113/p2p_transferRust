...
## 2026-08-01 - Insecure Directory Creation in Download Handlers
**Vulnerability:** The application was using the standard `tokio::fs::create_dir_all` and `std::fs::create_dir_all` for creating configuration and download directories across multiple modules (`p2p_core/src/http_share/websocket/handler.rs`, `p2p_core/src/transfer/receiver.rs`, `p2p_wan/src/receiver.rs`, `p2p_core/src/identity.rs`). This results in directories being created with default umask permissions, potentially allowing other users to access downloaded files or sensitive configuration data.
**Learning:** Default directory creation methods do not enforce restrictive permissions. When dealing with sensitive downloads or configuration keys (like `node_secret.key`), parent directories must also be secured to prevent unauthorized access.
**Prevention:** Always use dedicated secure directory creation helpers (like `create_secure_dir_all_async`) that utilize `tokio::fs::DirBuilder` with explicit `mode(0o700)` on Unix systems to ensure directories are restricted to the owner.
