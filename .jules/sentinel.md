...
## 2024-05-24 - [TOCTOU in Identity Key File Creation]
**Vulnerability:** A Time-of-Check to Time-of-Use (TOCTOU) vulnerability existed in `p2p_core/src/identity.rs` where the `node_secret.key` file was created using `.create(true).truncate(true)`. An attacker could pre-create this file (e.g., as a symlink or with world-readable permissions) before the Rust process opened it, causing the new sensitive key to be written with insecure permissions or to an attacker-controlled location.
**Learning:** Checking if a file exists before creating it (or relying on `.create(true).truncate(true)`) is not atomic and leaves a race condition window. Modifying file permissions via `set_permissions` *after* opening the file is also insufficient because existing file handles (if an attacker had already opened the file) are not revoked by changing the on-disk permissions.
**Prevention:** Always use `OpenOptions::new().create_new(true)` when creating sensitive files that must not already exist. This instructs the OS to atomically fail the file open operation if the file or a symlink already exists at that path, completely preventing TOCTOU replacement attacks.
