...
## 2024-05-01 - [Verify Existing File Permissions on Append]
**Vulnerability:** TOCTOU vulnerability due to lack of permission verification on existing files when appending. When a file is created using `OpenOptions::new().create(true)` or when a file is opened with `OpenOptions::new().append(true)` in Rust, the `.mode()` option doesn't apply to pre-existing files, leaving the file permissions vulnerable to tampering (e.g. `0o666`) by malicious actors who create the file before the application runs.
**Learning:** You must use `OpenOptions::new().create_new(true)` to guarantee atomic file creation with secure permissions. However, if the file already exists and needs to be appended to, `.mode()` does nothing for the pre-existing file. Setting `.set_permissions()` after opening an existing file is insufficient, as the malicious actor already holds an open file descriptor.
**Prevention:** When resuming or appending to existing files (where `create_new(true)` is not applicable), explicitly verify restrictive permissions (e.g., `0o600` on Unix) on the opened file handle's metadata (`file.metadata().await?.permissions().mode() & 0o777`) and reject the operation (return a `PermissionDenied` error) if tampered with. Do not attempt to fix permissions using `file.set_permissions()` after opening, as this introduces a TOCTOU vulnerability.
