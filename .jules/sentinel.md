...
## 2026-08-01 - [Read Operations Expose TOCTOU Vulnerability]
**Vulnerability:** The `load_or_generate` and `load_or_generate_sync` functions in `p2p_core/src/identity.rs` read the existing sensitive `node_secret.key` file using `fs::read(&key_path)`. This approach is vulnerable to a Time-of-Check to Time-of-Use (TOCTOU) race condition and fails to verify if the existing file has overly permissive permissions (e.g., world-readable) before reading its contents. An attacker could pre-create or replace this file with weak permissions, potentially exposing the secret key to unauthorized users on the same system.
**Learning:** Reopening or reading sensitive files based purely on path (using `fs::read` or similar path-based read operations) without explicitly verifying the file's metadata and permissions on the opened file descriptor allows potential information disclosure or TOCTOU issues.
**Prevention:** When reading existing sensitive files, always open the file first using `File::open` and immediately verify its permissions on the open file descriptor (`file.metadata()`) to ensure they match expected secure permissions (e.g., `0o600`). If they are too permissive, attempt to tighten them using `set_permissions` before reading the contents.
