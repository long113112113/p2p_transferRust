...
## 2024-05-30 - Insecure File Permissions on Resume
**Vulnerability:** The `open_secure_file` function in the P2P transfer module only enforced `0o600` permissions when creating a new file (offset = 0). When resuming a transfer (offset > 0), it relied on `OpenOptions::append(true)` without verifying permissions, allowing an attacker to pre-create a world-readable file that the application would then blindly append sensitive data to.
**Learning:** Time-of-Check to Time-of-Use (TOCTOU) issues and permission checks are easily missed in alternate code paths, such as file resuming/appending, even when the "create new" path is secure.
**Prevention:** Always verify and enforce restrictive file permissions (`0o600`) on the open file handle (e.g., using `file.set_permissions()`) after opening an existing file for modification, regardless of whether it's being truncated or appended.
