...
## 2024-05-24 - [Insecure File Permissions on Resume (Appending)]
**Vulnerability:** The `open_secure_file` function in `p2p_core/src/transfer/utils.rs` did not verify file permissions when opening an existing file for appending (`offset > 0`). This could allow an attacker to read or write to sensitive file data if they previously modified the file's permissions or pre-created the file with permissive modes before the application appended to it.
**Learning:** `OpenOptions::mode()` in Rust only sets permissions for newly created files. When resuming or appending to an existing file, the OS retains the file's existing permissions. You cannot rely on `OpenOptions` to secure an already-existing file against unauthorized access.
**Prevention:** When opening an existing file for appending or modifications (where `create_new(true)` cannot be used), explicitly query the file's metadata after opening and verify that the permissions are securely restricted (e.g., `0o600` on Unix). If the permissions are insecure, immediately close the file and return an error to prevent data leakage or tampering.
