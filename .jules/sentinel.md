...
## 2026-08-01 - [Insecure Permissions on Existing Secret Keys]
**Vulnerability:** The `load_or_generate` and `load_or_generate_sync` methods in `p2p_core/src/identity.rs` read the `node_secret.key` file using `fs::read(&key_path)` without verifying that the file permissions are secure (e.g., `0o600`). This allowed the application to silently load and use a secret key that could be read or modified by unauthorized users (e.g., if created manually with `0o644`).
**Learning:** Checking `.exists()` and blindly reading sensitive files assumes the environment is secure. Applications must actively verify that loaded secrets maintain the required access controls and fail securely if they do not. Furthermore, permission checks must not be overly strict (like `== 0o600`) as this rejects secure modes like `0o400` (read-only owner).
**Prevention:** Before reading existing sensitive files, explicitly check metadata permissions to ensure group and other users have no access (e.g., `perms.mode() & 0o077 != 0`). Log a warning and attempt to correct the permissions via `file.set_permissions()`, failing and refusing to load the data if the correction fails.
