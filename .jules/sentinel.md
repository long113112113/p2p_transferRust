...
## 2026-08-01 - [Insecure Download Directory Permissions in WAN Receiver]
**Vulnerability:** The `p2p_wan` crate was using the standard `tokio::fs::create_dir_all` to create download directories in `p2p_wan/src/receiver.rs`. This relies on the default umask and may create overly permissive directories, potentially allowing unauthorized access to files downloaded or transferred by the user.
**Learning:** Default directory creation in Rust does not apply restrictive modes, potentially leading to over-permissive directories for user data. Just like with configuration directories and local network transfers, WAN download directories should enforce strict permissions.
**Prevention:** Always use custom helper functions like `crate::config::create_secure_dir_all_async` (from `p2p_core`) that correctly set strict permissions (e.g. `0o700` on Unix) when creating directories intended to hold sensitive or user-downloaded files, even in separate crates within the workspace.
