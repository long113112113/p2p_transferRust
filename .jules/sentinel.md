...
## 2024-04-08 - [Fix TOCTOU vulnerability in identity key generation]
**Vulnerability:** The `IdentityManager::load_or_generate` and `IdentityManager::load_or_generate_sync` methods in `p2p_core/src/identity.rs` used an insecure `.exists()` check followed by `.create(true)` to generate the `node_secret.key` file.
**Learning:** This approach introduces a Time-of-Check to Time-of-Use (TOCTOU) vulnerability. An attacker could pre-create the file (e.g., as a symlink or with overly permissive permissions) between the check and the creation, leading to unauthorized access or modification of the sensitive key.
**Prevention:** Always use atomic file creation with secure permissions. In Rust, this means using `OpenOptions::create_new(true)` combined with `.mode(0o600)` (on Unix) to guarantee exclusive creation and correct permissions in a single operation. Handle the `AlreadyExists` error to fallback to reading the existing file.
