...
## $(date +%Y-%m-%d) - [Identity File Creation TOCTOU]
**Vulnerability:** The `load_or_generate` and `load_or_generate_sync` methods in `IdentityManager` used `OpenOptions::mode(0o600)` to set file permissions when generating the `node_secret.key` file. However, `mode` only applies when a *new* file is created. If the file already existed and was opened (for example, if it was manually created but empty, or if there was a TOCTOU race condition where an attacker pre-created the file before generation), the file would inherit the pre-existing, potentially insecure permissions (e.g. `0o666`).
**Learning:** `OpenOptions::mode` does not enforce metadata/permissions on an existing file when the `open` method is called. For sensitive files, especially identity keys that must remain secret, permissions must be explicitly set on the file handle after opening to ensure they are correct regardless of the file's prior existence.
**Prevention:** Always follow file opening/creation with an explicit `file.set_permissions(...)` call (with mode `0o600` for secrets) on Unix systems to guarantee the permissions are correctly restricted, preventing any TOCTOU vulnerability.
