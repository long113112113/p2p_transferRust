...
## 2024-05-29 - Missing Strict-Transport-Security Header
**Vulnerability:** The HTTP server (`p2p_core/src/http_share/server.rs`) was omitting the `Strict-Transport-Security` (HSTS) header in its `add_security_headers` middleware. This omission left clients vulnerable to protocol downgrade attacks, as browsers were not instructed to strictly enforce HTTPS connections.
**Learning:** Security header middlewares must comprehensively implement all standard headers. While CSP, X-Frame-Options, and X-Content-Type-Options were present, the absence of HSTS represented a critical gap in enforcing secure transport.
**Prevention:** Always verify the inclusion of `Strict-Transport-Security` alongside other standard security headers. Implement automated tests (e.g., `test_hsts_header_presence`) to assert the specific presence and correct formatting (e.g., `max-age=...; includeSubDomains`) of the HSTS header on server responses.
