...
## 2026-08-05 - Insecure File Appending via Permissive File Handling
**Vulnerability:** The `open_secure_file` function in `p2p_core/src/transfer/utils.rs` did not enforce file permissions when opening an existing file for appending (`offset > 0`). This left the system vulnerable to Time-of-Check to Time-of-Use (TOCTOU) attacks where an attacker could pre-create a file with permissive permissions (e.g., `0o666`), and the system would append to it without fixing or rejecting the insecure file.
**Learning:** `OpenOptions::mode(0o600)` only applies when creating *new* files. When opening an existing file for appending, its existing permissions remain untouched. Always verify that an existing file has strictly restricted permissions before writing sensitive data to it.
**Prevention:** Explicitly fetch and check file metadata immediately after opening an existing file for appending. Reject the operation with `std::io::ErrorKind::PermissionDenied` if the file's permissions are not exactly what is required (e.g., `0o600` on Unix).
