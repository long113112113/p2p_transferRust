...
**Vulnerability:** A Time-of-Check to Time-of-Use (TOCTOU) vulnerability existed when creating sensitive files (like `node_secret.key` in `identity.rs` and secure files in `utils.rs`). The code relied on `key_path.exists()` followed by `.create(true)` or post-creation `.set_permissions()` adjustments, leaving a race condition window where an attacker could pre-create a file with permissive modes (like `0o666`) to retain a file descriptor and compromise the file's security.
**Learning:** Checking for file existence before creation or modifying permissions after creation is not atomic and is vulnerable to race conditions on Unix systems.
**Prevention:** Always use atomic operations for file creation. Combine `OpenOptions::new().write(true).create_new(true)` with `.mode(0o600)` to ensure a file is securely and exclusively created with the correct permissions from the start.
