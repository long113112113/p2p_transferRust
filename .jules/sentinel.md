...
## 2026-06-13 - [Enforce Restrictive Download Directory Permissions in `p2p_wan`]
**Vulnerability:** Similar to `p2p_core`, the application was using the standard `tokio::fs::create_dir_all` to create the download directory in `p2p_wan/src/receiver.rs`. This relies on the default umask and may create overly permissive directories, potentially allowing unauthorized access to files downloaded or transferred by the user.
**Learning:** Standard library and `tokio` directory creation functions do not guarantee restrictive permissions across platforms, which is critical for user data security.
**Prevention:** Always use the custom helper function `p2p_core::config::create_secure_dir_all_async` to create download directories, ensuring strict `0o700` permissions on Unix systems.
