...
## 2026-08-05 - Insecure File Permissions when Appending
**Vulnerability:** The `open_secure_file` function correctly mitigated TOCTOU when creating new files by using `OpenOptions::create_new(true).mode(0o600)`. However, when appending to existing files (`offset > 0`), it simply used `OpenOptions::append(true)`. If an attacker pre-created the file with permissive permissions (e.g., world-readable `0o666`), appending to it would leak all subsequent data written to the file.
**Learning:** Checking `existence` before appending is insufficient and vulnerable to TOCTOU. When resuming transfers or appending to sensitive files, the application must explicitly verify that the existing file descriptor holds the required secure permissions (e.g., `0o600` on Unix).
**Prevention:** After opening a file for appending, immediately check its metadata permissions. If the permissions are less restrictive than required (e.g., not exactly `0o600`), return an error and reject the operation. Do not attempt to fix permissions after opening, as attackers may already have open file handles.
