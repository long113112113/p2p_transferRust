...
## 2024-05-30 - [TOCTOU Vulnerability in File Overwrites]
**Vulnerability:** A Time-of-Check to Time-of-Use (TOCTOU) vulnerability exists when creating sensitive files (e.g., `node_secret.key`) using `OpenOptions::new().write(true).create(true).truncate(true)` along with `.mode(...)`. If an attacker creates a permissive file (e.g., `0o666`) or a symlink between the `.exists()` check and the `.open()` call, `OpenOptions::mode` does not apply to the existing file. The application inherits the attacker's insecure permissions while saving the sensitive key.
**Learning:** In Rust, `OpenOptions::mode()` applies *only* to newly created files. When truncating or overwriting existing sensitive files, or when relying on `.create(true)` which can open an existing file, inherited permissions are an issue.
**Prevention:** Explicitly apply `set_permissions` immediately after successfully opening the file to guarantee restrictive permissions regardless of prior state, or exclusively use `create_new(true)` to fail if the file exists.
