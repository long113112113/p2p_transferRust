...
## 2024-05-30 - [TOCTOU Vulnerability in File Creation]
**Vulnerability:** A Time-of-Check to Time-of-Use (TOCTOU) vulnerability existed in `p2p_core/src/identity.rs` during the creation of sensitive node secret keys (`load_or_generate` and `load_or_generate_sync`). The code relied on checking `.exists()` followed by using `OpenOptions::create(true).truncate(true)`.
**Learning:** Checking file existence and subsequently creating it leaves a window open for race conditions (TOCTOU) where the file could be created by an attacker with malicious permissions or linked to another sensitive file between the check and the actual creation. Even with `OpenOptions::mode(0o600)`, if an attacker pre-creates the file (e.g., via a symlink or with open permissions), `.create(true)` will open the existing file rather than failing, and `mode(0o600)` only applies to newly created files, leaving the file with potentially insecure inherited permissions.
**Prevention:** Always use `OpenOptions::create_new(true)` when exclusively creating new sensitive files to ensure the file creation is an atomic operation and fails if the file already exists, eliminating the TOCTOU window. Furthermore, on Unix, always explicitly invoke `set_permissions` after opening the file to guarantee the restrictive `0o600` permissions are applied, effectively overwriting any inherited insecure permissions if `create(true)` must be used.
