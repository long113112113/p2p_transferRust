...
## 2026-10-24 - Insecure Identity Directory Creation
**Vulnerability:** The secret key file (`node_secret.key`) in `p2p_core/src/identity.rs` had its parent directory created using `fs::create_dir_all` and `std::fs::create_dir_all`, which use the default directory permissions (often `0o755` on Unix). This exposes the sensitive `node_secret.key` directory, allowing other local users to traverse into it. While the file itself was secured with `0o600`, securing the parent directory provides Defense in Depth.
**Learning:** File-level permissions are not enough when dealing with highly sensitive configurations. Directory-level permissions must also be locked down (`0o700`) to prevent unauthorized access or metadata leakage (like directory contents).
**Prevention:** Use the centralized `crate::config::create_secure_dir_all` helper function across the entire project for any configuration or sensitive directory creation.
