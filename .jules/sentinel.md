...
## 2025-10-24 - TOCTOU in Secure File Creation
**Vulnerability:** File creation functions across the application (`identity.rs`, `http_share/websocket/utils.rs`, `transfer/utils.rs`) used a pattern of `OpenOptions::create(true)` or `exists()` followed by file creation, and sometimes attempted to explicitly set secure file permissions using `set_permissions` *after* creation. This allows an attacker to pre-create the file before the server opens it, maintaining their handle, and thus maintaining insecure permissions on a file they control despite the server's post-open validation. This is a Time-of-Check to Time-of-Use (TOCTOU) vulnerability.
**Learning:** `OpenOptions::create(true)` will truncate existing files but does *not* enforce the `.mode()` restrictions on existing files. Additionally, `set_permissions` after opening does not revoke pre-existing file handles.
**Prevention:** Always use `OpenOptions::create_new(true)` in conjunction with `.mode(...)` for atomic and secure file creation on Unix. Never fallback to reading the file or checking `.exists()` if `create_new(true)` fails with an `AlreadyExists` error unless specifically handling an append operation, in which case ownership must be strictly validated.
