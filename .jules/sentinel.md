...
## 2024-05-28 - [Secure File Creation Fixes to Prevent TOCTOU]
**Vulnerability:** A Time-of-Check to Time-of-Use (TOCTOU) vulnerability where `create(true).truncate(true)` was used with `OpenOptions` in rust to create sensitive files such as node secrets or downloaded configurations. Because file permissions with `PermissionsExt` cannot be enforced securely to existing files when using `create(true)`, an attacker could create the file beforehand with malicious permissions (e.g., `0o666`), forcing the new file handles to use these pre-existing permissions before the application had a chance to `chmod` it down to `0o600`.
**Learning:** Checking or setting permissions after file creation or opening an existing file via truncation does not inherently limit access if an attacker has already opened the file. Specifically in Rust Unix, `options.mode(0o600)` only correctly applies to *newly* created files.
**Prevention:** To prevent this TOCTOU vulnerability, ensure that sensitive files are always created securely by explicitly deleting any existing target (`remove_file`) and enforcing strict creation with `.create_new(true)`. By using `create_new(true)`, the system call is guaranteed to fail if the file exists, avoiding falling back to an insecure truncation operation, and reliably sets `.mode(0o600)` on the atomic creation process.
