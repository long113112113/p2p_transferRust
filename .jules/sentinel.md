...
## 2026-08-15 - Insecure File Permissions via Append TOCTOU
**Vulnerability:** The `open_secure_file` function failed to verify file permissions when opening an existing file for appending (`offset > 0`). An attacker could pre-create a file with permissive permissions (e.g., 0o666), and the server would blindly write sensitive data to it.
**Learning:** When resuming a transfer or appending to an existing file, you cannot rely on creation-time flags (since `create_new` isn't used). The open file handle must be explicitly checked to ensure it retains secure permissions.
**Prevention:** When opening an existing file for writing (append mode), always inspect the file metadata on the opened handle and reject the operation (fail securely) if the permissions are not restrictive (e.g., 0o600). Do not attempt to fix permissions post-open, as the attacker may already hold an open file descriptor.
