...
## 2024-05-03 - TOCTOU and Insecure Append Operations
**Vulnerability:** The `open_secure_file` function in `p2p_core/src/transfer/utils.rs` did not verify the permissions of an existing file when opening it in `append` mode (`offset > 0`). If an attacker pre-created the file with permissive permissions (e.g., 0o666), the file was opened and appended to, but the insecure permissions were preserved, exposing the appended content.
**Learning:** While `create_new(true)` handles atomicity for *new* files, appending to *existing* files requires explicit permission validation on the acquired file descriptor to prevent TOCTOU vulnerabilities where an attacker pre-creates the file. Changing permissions *after* opening is insufficient as the attacker may already hold a file descriptor.
**Prevention:** When opening an existing file for appending or modification, verify its metadata (e.g., `permissions().mode() & 0o777`) on the open file handle. If the permissions are less restrictive than required (e.g., `!= 0o600`), reject the operation and return an error.
