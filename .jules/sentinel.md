...
## 2026-08-01 - TOCTOU and Insecure Append Operations
**Vulnerability:** The `open_secure_file` helper intended to ensure files are accessed securely. However, for append operations (`offset > 0`), it used `options.append(true)` without validating the permissions of the existing file. An attacker could pre-create a file with permissive permissions (e.g., world-readable) and then trigger an append operation, causing sensitive data to be written to the insecurely configured file.
**Learning:** Checking permissions is critical not just during creation but also when opening existing files for modification. If a function is expected to provide a "secure file", it must verify that existing files meet the required security posture before writing data to them.
**Prevention:** When appending to or opening an existing file where secure permissions are expected, explicitly check the file's metadata after opening and reject the operation if the permissions are not restrictive (e.g., `0o600`).
