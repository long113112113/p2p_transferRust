...
## 2025-04-12 - [Critical] Fix TOCTOU vulnerability in secure file creation
**Vulnerability:** Files intended to be created with secure permissions (e.g. `0o600` via `options.mode(0o600)`) were being opened with `options.create(true).truncate(true)`. If an attacker pre-created the file or placed a symlink at the target path, the application would unknowingly truncate and follow the existing file/symlink rather than creating a new file with strict atomic ownership and permissions.
**Learning:** `std::fs::OpenOptions::create(true)` is not atomic against existing files. Using `set_permissions` after the fact is vulnerable to a race condition (TOCTOU).
**Prevention:** Always use `options.create_new(true)` with `.mode(...)` for secure atomic creation on Unix. If overwriting an existing file safely is intended, first unlink the file (e.g. `remove_file`) and then call `create_new(true)` rather than relying on `.truncate(true)`.
