...
## 2026-08-01 - TOCTOU via Insecure Directory Creation
**Vulnerability:** The codebase occasionally used standard `tokio::fs::create_dir_all` or `std::fs::create_dir_all` when creating directories intended to contain sensitive files (like identity keys or configuration files). Even when files inside the directory were created with `0o600` permissions, a parent directory lacking `0o700` strictness could expose sensitive files to other users or lead to Time-of-Check to Time-of-Use (TOCTOU) issues where an attacker creates a malicious directory structure first.
**Learning:** Directories holding sensitive information must have restrictive permissions (`0o700` on Unix) from creation. Standard directory creation logic does not provide these constraints securely by default.
**Prevention:** Rely on custom, explicitly secure async helpers like `create_secure_dir_all_async` when preparing configuration or download paths, ensuring all intermediate directories apply the `0o700` mode upon creation.
