...
## 2026-08-16 - File Permission Bypass on Append
**Vulnerability:** In `open_secure_file`, when appending to a file (`offset > 0`), the code correctly did not use `create_new(true)`. However, it failed to verify the existing file's permissions. An attacker could pre-create the target file with permissive permissions (like `0o666`), and subsequent appends by the server would unknowingly write sensitive data into this world-readable file, bypassing the `0o600` security intent.
**Learning:** While `create_new(true)` is the correct defense against TOCTOU when creating *new* files, appending to existing files requires an explicit permission check on the opened file handle. You cannot rely on `OpenOptions::mode` for existing files.
**Prevention:** When opening existing files for writing/appending sensitive data, always check `metadata().permissions().mode()` on the opened file descriptor. If the permissions are less restrictive than required, abort the operation. Do not attempt to "fix" it with `set_permissions` as the attacker may already have an open handle to the file.
