...
## 2026-08-01 - [Append Operations Expose TOCTOU Vulnerability]
**Vulnerability:** When reopening existing files for reading or appending, failing to verify exact permissions allows attackers to pre-create files with overly permissive settings (e.g., world-readable) leading to Time-of-Check to Time-of-Use (TOCTOU) issues where file data could be intercepted or manipulated.
**Learning:** Checking that `perms.mode() & 0o777 != 0o600` is overly strict for read-only keys, rejecting valid `0o400` states. The correct approach is checking that group/other access is strictly denied (`perms.mode() & 0o077 != 0`). Attempting to `set_permissions` afterwards provides a "best-effort" fix but cannot revoke existing open handles an attacker may hold.
**Prevention:** Instead of requiring an exact match like `0o600`, verify group/other access is unset by masking with `0o077`. Log warnings and apply fixes using `set_permissions` while acknowledging that true atomic security requires fresh creation via `create_new(true)` with `.mode(...)` when the file doesn't exist yet.
