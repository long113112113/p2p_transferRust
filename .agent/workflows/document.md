...
### SecretKey
Để giữ Node ID cố định (bền vững qua các lần khởi động), cần quản lý `SecretKey`.
...
// Serialize ra bytes (32 bytes)
let bytes = secret_key.to_bytes();
// Lưu `bytes` xuống file/db...
...
- `EndpointId` chính là `PublicKey` tương ứng của `SecretKey`.
- `node_id` sẽ không đổi miễn là `SecretKey` được giữ nguyên.

<!-- Edit check successful -->
